...
/* LE CHOIX D'ICÔNES (créer SON kpi) : une douzaine de lignes claires, tappables
   à l'essayage et à la retouche. L'ORDRE et l'appartenance viennent de
   ICONES_IDS (pur, partagé avec actions.js/les gardes) ; ici on associe chaque
   id à son glyphe. `iconeChoisie(id)` → le SVG, ou null. */
const SVG_PAR_ID = {
  portefeuille: ICONE_DOMAINE.budget,
  bouclier: ICONE_DOMAINE.coussin,
  vagues: ICONE_DOMAINE.saisonnier,
  recu: ICONE_DOMAINE.impot,
  courbe: ICONE_DOMAINE.patrimoine,
  pieces: ICONE_KPI.solde_mois,
  coussin: ICONE_KPI.mois_couverts,
  cible: (<svg {...S}><circle cx="12" cy="12" r="8.5" /><circle cx="12" cy="12" r="4.5" /><circle cx="12" cy="12" r="1" /></svg>),
  eclair: I_ECLAIR,
  etoile: (<svg {...S}><path d="M12 3l2.5 5.4 5.9.7-4.4 4 1.2 5.8L12 16l-5.2 2.9 1.2-5.8-4.4-4 5.9-.7L12 3z" /></svg>),
  coeur: (<svg {...S}><path d="M12 20s-7.5-4.6-9.3-9.3C1.5 7.5 3.6 4.5 6.8 4.5c2 0 3.6 1.1 4.4 2.7.8-1.6 2.4-2.7 4.4-2.7 3.2 0 5.3 3 4.1 6.2C17.5 15.4 12 20 12 20z" /></svg>),
  drapeau: (<svg {...S}><path d="M5 21V4" /><path d="M5 4c4-2 7 2 11 0v9c-4 2-7-2-11 0" /></svg>),
  maison: (<svg {...S}><path d="M3 11.5 12 4l9 7.5" /><path d="M5 10v10h14V10" /><path d="M10 20v-5h4v5" /></svg>),
  soleil: (<svg {...S}><circle cx="12" cy="12" r="4.5" /><path d="M12 2.5v3M12 18.5v3M2.5 12h3M18.5 12h3M5 5l2 2M17 17l2 2M19 5l-2 2M7 17l-2 2" /></svg>),
}
export const ICONES_CHOIX = ICONES_IDS.map((id) => ({ id, svg: SVG_PAR_ID[id] }))
