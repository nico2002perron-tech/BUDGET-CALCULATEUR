...
console.log('\n— Couche de compréhension : la situation objectif_epargne compose un bloc chaine —')
ok(estConnu('chaine') === true, 'le bloc chaine est connu (schema + registre)')
const rcObj = composerRecette('objectif_epargne')
ok(rcObj.blocs.some((b) => b.type === 'chaine'), 'objectif_epargne compose un bloc chaine')
ok(rcObj.blocs.every((b) => estConnu(b.type)), 'tous les blocs de objectif_epargne sont connus')
